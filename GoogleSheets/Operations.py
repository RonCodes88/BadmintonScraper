    ...
    if all(col == '' for col in date_columns):
        ...
        if match_date < datetime.strptime(df.columns[7], "%m/%d/%Y"):
            ...
            for i in range(len(date_columns) - 1):
                if datetime.strptime(date_columns[i], "%m/%d/%Y") <= match_date <= datetime.strptime(
                        date_columns[i + 1], "%m/%d/%Y"):
...
def insert_player_alphabetically(player, new_rating, df):
    ...
    new_row = pd.DataFrame(
        {'Player Ordered': [player], 'Latest Rating Ordered': [new_rating], 'Player Alphabetical': [player],
         'Latest Rating': [new_rating], 'Player': [player], 'Initial Rating': 1300})
...
def update_player_rating(player, new_rating, new_rating_date, date, elo_point, df):
    ...
    # Insert new column at the end of the dates
        ...
        if new_rating_date not in df.columns:
            df.insert(loc=len(df.columns), column=new_rating_date,
                      value=None)  # Use the string version of date for column name
...
def sort_by_latest_rating(df):
    ...
    df_ordered_rating = df[['Player Ordered', 'Latest Rating Ordered']].sort_values(by='Latest Rating Ordered',
                                                                                    ascending=False)
...
if __name__ == "__main__":
    ...
    df_singles_to_sheet = [df_singles.columns.values.tolist()] + df_singles.where(pd.notnull(df_singles),
                                                                                  '').values.tolist()
    df_doubles_to_sheet = [df_doubles.columns.values.tolist()] + df_doubles.where(pd.notnull(df_doubles),
                                                                                  '').values.tolist()
